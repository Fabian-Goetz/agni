...
// ---- Activity cards (game rule data, keyed by equipment id) ------------------
// Deliberately a separate CSV from the Geräte-Katalog: difficulty and Tabu-Wörter
// are rules about a game, not facts about a Gerät (ADR-0004 keeps layer 1 clean).
const DIFFICULTIES = ['Leicht', 'Mittel', 'Schwer'];
const PERFORM_MODES = ['Beschreiben', 'Zeichnen', 'Pantomime'];
const equipmentIds = new Set(equipment.map((e) => e.id));

const SEED_ACTIVITY_CARDS = csv('activity-cards-v1.csv').map((r) => {
  if (!equipmentIds.has(r.equipment_id))
    throw new Error(`activity card references unknown equipment: ${r.equipment_id}`);
  if (!DIFFICULTIES.includes(r.schwierigkeit))
    throw new Error(`activity card '${r.equipment_id}': bad difficulty '${r.schwierigkeit}'`);

  const card = { equipmentId: r.equipment_id, difficulty: r.schwierigkeit };
  const taboo = (r.tabu_woerter || '').split(';').map((s) => s.trim()).filter(Boolean);
  if (taboo.length) card.taboo = taboo;
  const excl = (r.ausgeschlossene_modi || '').split(';').map((s) => s.trim()).filter(Boolean);
  for (const m of excl)
    if (!PERFORM_MODES.includes(m))
      throw new Error(`activity card '${r.equipment_id}': bad mode '${m}'`);
  if (excl.length) card.excludeModes = excl;
  return card;
});
{
  const seen = new Set();
  for (const c of SEED_ACTIVITY_CARDS) {
    if (seen.has(c.equipmentId)) throw new Error(`duplicate activity card: ${c.equipmentId}`);
    seen.add(c.equipmentId);
  }
  // A difficulty with no cards would make that gamble undealable on the board.
  for (const d of DIFFICULTIES)
    if (!SEED_ACTIVITY_CARDS.some((c) => c.difficulty === d))
      throw new Error(`no activity cards at difficulty '${d}'`);
}
...
const cardsTs = `${banner}import { ActivityCard } from '../models/activity-card';

export const SEED_ACTIVITY_CARDS: ActivityCard[] = ${JSON.stringify(SEED_ACTIVITY_CARDS, null, 2)};
`;
writeFileSync(join(ROOT, 'src/app/core/seed/seed-activity-cards.ts'), cardsTs);
...
const acRows = SEED_ACTIVITY_CARDS.map(
  (c) => `  (${s(c.equipmentId)}, ${s(c.difficulty)}, ${arr(c.taboo)}, ${arr(c.excludeModes)})`,
).join(',\n');
...
insert into activity_cards (equipment_id, difficulty, taboo, exclude_modes) values
${acRows}
on conflict (equipment_id) do update set
  difficulty = excluded.difficulty, taboo = excluded.taboo,
  exclude_modes = excluded.exclude_modes;
`;
writeFileSync(join(ROOT, 'docs/supabase/seed.sql'), sql);

const byDifficulty = DIFFICULTIES.map(
  (d) => `${d} ${SEED_ACTIVITY_CARDS.filter((c) => c.difficulty === d).length}`,
).join('/');
console.log(`seed generated: ${equipment.length} equipment, ${SEED_VEHICLE_TYPES.length} vehicle types (LF placements: ${lfLoadout.length}), ${SEED_ACTIVITY_CARDS.length} activity cards (${byDifficulty})`);
